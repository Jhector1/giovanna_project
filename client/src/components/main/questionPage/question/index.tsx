...
import { Question, RootsState } from '../../../../types';
  ...
  return (
    <div
      ...
      <div className='question_mid'>
        <div className={sidebarMode || 'postTitle'}>{q.title}</div>
        <div className='question_tags'>
          {q.tags.map((tag, idx) => (
            <button
              key={idx}
              style={{ color: buttonMode === 'colorChoiceDarkGrayButton' ? 'white' : '' }}
              className={`${buttonMode || 'question-btn'} question_tag_button`}
              onClick={e => {
                e.stopPropagation();
                clickTag(tag.name);
              }}>
